...
Перем ТаблицаПровайдеровПолученияПараметров; // ТаблицаЗначений Колонки: Имя, Класс, Приоритет, Настройки
...
Процедура ПриСозданииОбъекта()
	...
	ТаблицаПровайдеровПолученияПараметров = Новый ТаблицаЗначений;
	ТаблицаПровайдеровПолученияПараметров.Колонки.Добавить("Имя") // Представление 
	ТаблицаПровайдеровПолученияПараметров.Колонки.Добавить("Класс") // ИмяКласса или Инстанс Класс
	ТаблицаПровайдеровПолученияПараметров.Колонки.Добавить("Приоритет") // Число от 0 до 10
	ТаблицаПровайдеровПолученияПараметров.Колонки.Добавить("Настройки") // Структура

	ДобавитьПровайдерПолученияПараметров("json", "ПровайдерПараметровJSON", Новый Структура, 0);

	ОбщаяНастройка = Новый Структура("ФайлПараметров");

КонецПроцедуры

Процедура УстановитьПровайдерПолученияПараметров(Знач Имя, Знач Класс, Знач Настройки = Неопределено, Знач Приоритет = Неопределено) Экспорт
	
	ТаблицаПровайдеровПолученияПараметров.Очистить();
	
	ДобавитьПровайдерПолученияПараметров(Имя, Класс, Настройки, Приоритет);

КонецПроцедуры

Процедура ДобавитьПровайдерПолученияПараметров(Знач Имя, Знач Класс, Знач Настройки = Неопределено, Знач Приоритет = 0) Экспорт
	
	НовыйПровайдер = ТаблицаПровайдеровПолученияПараметров.Добавить();
	НовыйПровайдер.Имя = Имя;
