...
Процедура ОбработатьПараметрыРекурсивно(Источник, Знач ИмяФайлаПараметров)
	...
	Для каждого Ключ Из КлючиКДополнительномуЧтению Цикл
		...
		ПутьКФайлуПараметров = Источник[Ключ];
		
		ПрочитатьФайлИзЗначенияПараметра(Ключ, ПутьКФайлуПараметров, ПутьКаталогаФайла, Источник);
...
Процедура ПрочитатьФайлИзЗначенияПараметра(Знач Ключ, Знач ПутьКФайлуПараметров, Знач ПутьКаталогаФайла, Приемник)

	Лог.Отладка("Нашли ключ файла <%1>, значение <%2>, путь каталога-родителя <%3>", Ключ, ПутьКФайлуПараметров, ПутьКаталогаФайла);

	ПутьФайла = ОбъединитьПути(ПутьКаталогаФайла, ПутьКФайлуПараметров);
