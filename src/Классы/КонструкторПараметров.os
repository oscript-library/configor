...
Перем НужноСоздатьПустойОбъект; // Булево, признак необходимости создать пустой набор полей
...
//   КонструкторПараметров - ссылка на новый элемент класса <КонструкторПараметров>
//
...
//
// Возвращаемое значение:
//   КонструкторПараметров - ссылка на текущий элемент класса <КонструкторПараметров>
//
...
//
// Возвращаемое значение:
//   ПолеКонструктораПараметров - ссылка на текущий элемент класса <ПолеКонструктораПараметров>
//
...
//   СоздаватьПоле       - Булево - признак создания пустого поля для объекта (по умолчания истина)
//
// Возвращаемое значение:
//   ПолеКонструктораПараметров - ссылка на текущий элемент класса <ПолеКонструктораПараметров>
//
Функция ПолеОбъект(Знач ИмяПоля, Знач ОбъектПоля, Знач СоздаватьПоле = Истина) Экспорт
	...
	Возврат Поле(ИмяПоля, ТипПоля, ОбъектПоля, , ОбъектПоля, СоздаватьПоле);
...
Функция Поле(Знач ИмяПоля,
			 ...
			 Знач ОбъектПоля = Неопределено,
			 Знач СоздаватьПоле = Истина) Экспорт
	...
	НовоеПолеПараметров.ЗначениеПоУмолчанию = ЗначениеПоУмолчанию;
...
Функция ВСтруктуру() Экспорт
	...
	Для каждого КлючЗначение Из Настройки Цикл
		...
		Если ЗначениеПоляЗаполнено(Значение) 
			...
			ИсходящаяСтруктура.Вставить(ИмяПоля, ЗначениеКлюча);
...
Функция ВСоответствие() Экспорт
	...
	Для каждого КлючЗначение Из Настройки Цикл
		...
		Если НадоСоздатьПоляНастройки(ИмяПоля) Тогда
			...
			СоответствиеРезультат.Вставить(ИмяПоля, ЗначениеКлюча);
...
//
// Возвращаемое значение:
//   КонструкторПараметров - ссылка на текущий объект <КонструкторПараметров>
//
Функция ИзСтруктуры(Знач ВходящаяСтруктура)
...
//
// Параметры:
//   ПолеПараметров - ПолеКонструктораПараметров - класс <ПолеКонструктораПараметров> для чтения синонимов
//
...
Функция НадоСоздатьПоляНастройки(Знач ИмяПоля)
	
	ПолеИндекса = НайтиПолеВИндексеПолей(ИмяПоля);

	Если ПолеИндекса = Неопределено Тогда
		Возврат МожетСодержатьПроизвольныеПоля;
	КонецЕсли;

	Возврат ПолеИндекса.НадоСоздавать();
...
Процедура ПриСозданииОбъекта()
	...
	НужноСоздатьПустойОбъект = Истина;
