...
Перем ИндексРеализованныхИнтерфейсов;
...
Процедура УстановитьКлассПриемник(КлассПараметров) Экспорт
	...
	ОбновитьИндексРеализованныхИнтерфейсов(КлассПриемникПараметров);
...
Функция ПроверитьКлассПровайдера(КлассЧтенияПараметров)
	...
	Если НЕ НеобходимыйИнтерфейсЕсть Тогда
		Лог.КритичнаяОшибка("Класс <%1> не реализует необходимой функции <%2>",
							КлассЧтенияПараметров,
							"ПрочитатьПараметры(ПараметрыПровайдеры)" );
	КонецЕсли;
...
Процедура ВыполнитьЧтениеДляПровайдера(ИдентификаторПровайдера, КлассПровайдера, ПараметрыПровайдера)
	...
	Лог.Отладка("Провайдер <%1> вернул <%2> параметров", 
				ИдентификаторПровайдера,
...
Процедура ВыгрузитьПараметрыВКлассПриемник()
	...
	Если ТипЗнч(КлассПриемникПараметров) = Тип("КонструкторПараметров") Тогда

		КлассПриемникПараметров.ИзСоответствия(ПрочитанныеПараметры);
...
Процедура ЗаполнитьСтруктуруПараметров(СтруктураПараметров)
	...
	Для каждого КлючЗначение Из СтруктураПараметров Цикл
		...
		ПриУстановкеЗначенияПараметра(ИмяПараметра, ИмяКлюча, НовоеЗначение, СтандартнаяОбработка);
...
Процедура ПриУстановкеЗначенияПараметра(Знач ПолныйПутьКлюча, Знач ИмяКлюча, Значение, СтандартнаяОбработка)

	ЕстьНужныйМетод = Истина; // Добавить поверку 1 раз в кеш
	Если ЕстьНужныйМетод Тогда
		КлассПриемникПараметров.ПриУстановкеЗначенияПараметра(ПолныйПутьКлюча, ИмяКлюча, Значение, СтандартнаяОбработка);
	КонецЕсли;
...
Функция ОбработатьЗначениеРекурсивно(Знач ЗначениеСтруктуры, Знач ЗначениеПараметра, Знач ИмяПараметра = "")
	...
	ИначеЕсли ЭтоСоответствие(ЗначениеСтруктуры) Тогда
		Возврат ОбработатьЗначениеСоответствие(ЗначениеСтруктуры, ИмяПараметра);
	ИначеЕсли ЭтоСтруктура(ЗначениеСтруктуры) Тогда
		Возврат ОбработатьЗначениеСтруктуру(ЗначениеСтруктуры, ИмяПараметра);
	Иначе
...
Функция ОбработатьЗначениеМассив(Знач МассивСтруктуры, Знач МассивЗначений, Знач ИмяРодителя = "")
	...
	Для ИндексЗначения = 0 По МассивЗначений.ВГраница() Цикл
		...
		ПриУстановкеЗначенияПараметра(ИмяПараметра, ИндексЗначения, НовоеЗначение, СтандартнаяОбработка);
...
Функция ОбработатьЗначениеСоответствие(Знач СтруктураПриемник, Знач ИмяРодителя = "")
...
Функция ОбработатьЗначениеСтруктуру(Знач СтруктураПриемник, Знач ИмяРодителя = "")
...
Процедура ПриСозданииОбъекта(Знач Провайдеры = "json")
	...
	ИнтерфейсПровайдера = Новый ИнтерфейсКласса;
	ИнтерфейсПровайдера.Ф("ПрочитатьПараметры", 1);

	ИнтерфейсПриемника = Новый ИнтерфейсКласса;
	ИнтерфейсПриемника.П("УстановитьНастройки", 1);
	ИнтерфейсПриемника.Ф("ПолучитьНастройки");

	ИнтерфейсКонвертацииВСтруктуру = Новый ИнтерфейсКласса;
	ИнтерфейсКонвертацииВСтруктуру.П("ПриПолученииПараметраНастройки", 1);

	ВнутреннийКонструкторПараметров = ПолучитьКонструкторПараметров();
	
КонецПроцедуры

Процедура ОбновитьИндексРеализованныхИнтерфейсов(КлассОбъект)
	
	ПроверкаКласса = Новый ПроверкаКласса(КлассОбъект);
	
	РезультатПроверки = ПроверкаКласса.РеализуетИнтерфейс(ИнтерфейсПриемника, Истина);

	ИндексРеализованныхИнтерфейсов.Вставить("ИнтерфейсПриемник", РезультатПроверки);

	РеализованныеМетоды = ПроверкаКласса.РеализованныеМетодыИнтерфейса(ИнтерфейсКонвертацииВСтруктуру);

	ИндексРеализованныхИнтерфейсов.Вставить("ИнтерфейсКонвертацииВСтруктуру", РеализованныеМетоды);

КонецПроцедуры

Процедура ПриверитьИНтерфейсПровайдера(КлассПровайдера)

	ИнтерфейсРеализован(ИнтерфейсПровайдера, КлассПровайдера, Истина);
...
Функция ИнтерфейсРеализован(Интерфейс, КлассОбъект, ВыдатьОшибку = Ложь)
	...
	ПроверкаКласса = Новый ПроверкаКласса(КлассОбъект);
	Возврат ПроверкаКласса.РеализуетИнтерфейс(Интерфейс, ВыдатьОшибку);
